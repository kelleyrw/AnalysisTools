{
    ...
    if (cmssw)
    {
        std::cout << "Loading AnalysisTools" << std::endl;
        ...
        gSystem->AddIncludePath(Form("-I\"%s/src/AnalysisTools/RootTools/interface\"", cmssw_base.Data()));
        gSystem->Load(Form("%s/libAnalysisToolsRootTools.%s", cmssw_lib.Data(), dy_suffix.Data()));

        gSystem->AddIncludePath(Form("-I\"%s/src/AnalysisTools/LanguageTools/interface\"", cmssw_base.Data()));
